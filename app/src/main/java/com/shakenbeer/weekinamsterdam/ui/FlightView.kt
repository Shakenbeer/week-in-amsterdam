...
class FlightView @JvmOverloads constructor(
    ...
    init {
        val a = context.theme.obtainStyledAttributes(attrs, R.styleable.FlightView, 0, 0)
        try {
            color = a.getColor(R.styleable.FlightView_color, Color.BLACK)
            flightDuration = a.getInt(R.styleable.FlightView_duration, DEFAULT_DURATION_MILLIS).let {
                when {
